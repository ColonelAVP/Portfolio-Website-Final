...
st.write("Made by - Atherv Patil")
st.write("### Write the name of a City and select the Temperature Unit")
...
def plot_line(days, min_t, max_t):
    ...
    plt.plot(days, max_t, color='black', linestyle='dashdot', linewidth=1, marker='o', markerfacecolor='green',
             markersize=7)
    plt.plot(days, min_t, color='black', linestyle='dashdot', linewidth=1, marker='o', markerfacecolor='blue',
             markersize=7)
...
def weather_detail(place, unit, g_type):
    ...
    for obj in days:
        ta = (obj.strftime("%m/%d"))
        st.write(f'### \v {ta} :\t  ({max_t[i]} - {min_t[i]})')
        i += 1
